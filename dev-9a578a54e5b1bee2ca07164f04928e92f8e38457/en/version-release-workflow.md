# Version Release Workflow - Draft PH Core Implementation Guide v0.2.0
...
1. **CI/CD build**→ runs SUSHI + IG Publisher, produces`output/`
1. **GitHub Pages publish**→ versioned directory (`gh-pages/X.Y.Z/`)
1. **FHIR IG Registry**→ PR to`FHIR/ig-registry``fhir-ig-list.json`
1. **Package Registry**→`packages.fhir.org`indexes the published package
...
1. **Install dependencies**→ Node.js, SUSHI, Java 21, Jekyll, Graphviz
1. **Download IG Publisher**→`input-cache/publisher.jar`
1. **Run IG Publisher**→`java -jar publisher.jar ig.ini`
...
1. Edit`fhir-ig-list.json`and add an entry:
...
1. Submit a PR to`FHIR/ig-registry`
...
1. Confirm`gh-pages/X.Y.Z/`renders at`https://UP-Manila-SILab.github.io/ph-core/X.Y.Z/`
1. Confirm`https://UP-Manila-SILab.github.io/ph-core/current/`redirects to the latest release.
1. Confirm`package-list.json`is valid (FHIR tooling can parse it).
1. Confirm downstream IGs can resolve the dependency:
...
For a chronological list of all releases, see the [Changes](changes.md) page.
