...
def validate_fhir_resources(server_url, input_dir, output_dir):
    ...
    # Iterate over all JSON files in the input directory that match '*-example.json'
    for filename in os.listdir(input_dir):
        if fnmatch.fnmatch(filename, '*-example.json'):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send FHIR validate POST requests for JSON files matching '*-example.json'.")
