...
def generate_bubble_mindmap_html(csv_file_path, output_html_path):
    ...
    # Helper function to recursively convert tree to JS object notation
    def tree_to_js_obj(node, prefix="", depth=1):
        result = "["
        items = list(node.items())
        for idx, (key, value) in enumerate(items):
            sanitized_key = sanitize_for_mermaid(key)
            full_key = f"{prefix}.{sanitized_key}" if prefix else sanitized_key
            count = node_counts.get(full_key, 1)
            
            result += f"""{{name: "{sanitized_key}", value: {count}"""
            
            if value:  # If this node has children
                result += ", children: "
                result += tree_to_js_obj(value, full_key, depth + 1)
            
            result += "}"  # Close the object
            if idx < len(items) - 1:  # Add comma if not the last item
                result += ","
        
        result += "]"  # Close the array
        return result
...
<body>
    ...
    # Convert the tree to JavaScript object notation
    js_data = tree_to_js_obj(tree)
    
    # Complete the HTML with the generated data
